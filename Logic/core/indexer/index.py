...
class Index:
    ...
    def index_stars(self):
        ...
        #         TODO
        stars_index = defaultdict(dict)

        for doc in self.preprocessed_documents:
            doc_stars = [word for star in doc['stars'] for word in star.split()]
            for term in doc_stars:
                term_freq = defaultdict(int)
                term_freq[doc['id']] = doc_stars.count(term)
                stars_index[term].update(term_freq)
    ...
    def index_genres(self):
        ...
        #         TODO
        genres_index = defaultdict(dict)

        for doc in self.preprocessed_documents:
            doc_genres = [word for genre in doc['genres'] for word in genre.split()]
            for term in doc_genres:
                term_freq = defaultdict(int)
                term_freq[doc['id']] = doc_genres.count(term)
                genres_index[term].update(term_freq)
    ...
    def index_summaries(self):
        ...
        summaries_index = defaultdict(dict)

        for doc in self.preprocessed_documents:
            all_summaries = [word for summary in doc['summaries'] for word in summary.split()]
            for term in all_summaries:
                term_freq = defaultdict(int)
                term_freq[doc['id']] = all_summaries.count(term)
                summaries_index[term].update(term_freq)
    ...
    def get_posting_list(self, word: str, index_type: str):
        ...
        try:
            posting_list = self.index[index_type][word]
    ...
    def add_document_to_index(self, document: dict):
        ...
        for index_type in self.index:
            if index_type == Indexes.DOCUMENTS.value:
                self.index[index_type][doc_id] = document
            else:
                content = document.get(index_type.lower(), [])
                for term in content:
    ...
    def remove_document_from_index(self, document_id: str):
        ...
        for index_type in self.index:
            if index_type == Indexes.DOCUMENTS.value:
                del self.index[index_type][document_id]
            else:
    ...
    def store_index(self, path: str, index_type: str):
        ...
        with open(os.path.join(path, f"{index_type}_index.json"), 'w') as f:
            json.dump(index_to_store, f, indent=4)
